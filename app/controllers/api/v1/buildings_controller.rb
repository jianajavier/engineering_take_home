class Api::V1::BuildingsController < ApplicationController
  ...
  def index
    ...
    render json: {
      ...
      buildings: buildings.map { |building|
        ...
        # Add client custom fields with their values
        building_json.merge!(
          building_custom_field_values: client_custom_fields.map do |custom_field|
            {
              field_name: custom_field.name,
              value: building.building_custom_field_values.find { |v| v.client_custom_field_id == custom_field.id }&.value
            }
          end
        )

        building_json # Return the modified hash for this building
      }
    }, status: :ok
  end
  ...
  def create_custom_field_values(building)
    ...
    # If there are any errors, render them as a response
    unless errors.empty?
  ...
  def valid_custom_field_value?(client_custom_field, value)
